...
// ── Soil texture backgrounds (CSS gradients simulating real soil) ─────────────
const SOIL_TEXTURES = {
  sand: `repeating-linear-gradient(
    45deg,
    #e8d5a3 0px, #e8d5a3 2px,
    #d4b896 2px, #d4b896 4px,
    #c9a87c 4px, #c9a87c 6px,
    #e0c898 6px, #e0c898 8px
  )`,
  clay: `repeating-linear-gradient(
    120deg,
    #8b5e3c 0px, #8b5e3c 3px,
    #7a4f30 3px, #7a4f30 6px,
    #9c6b45 6px, #9c6b45 9px,
    #6e4428 9px, #6e4428 12px
  )`,
  silt: `repeating-linear-gradient(
    90deg,
    #9e8668 0px, #9e8668 2px,
    #b09478 2px, #b09478 5px,
    #8a7258 5px, #8a7258 7px,
    #a08870 7px, #a08870 10px
  )`,
};
...
const ZONES = [
  {
    ...
    name: "Zone 1",
    soil: "sand",
    soilLabel: "Sandy Soil",
    color: "#e74c3c",
    lightColor: "#fdf2f2",
    borderColor: "#e74c3c",
    crop: "Root vegetables (carrots, potatoes)",
    riskLevel: "High",
    data: {
      soil_moisture: 0.20,
  ...
  },
  {
    ...
    name: "Zone 2",
    soil: "silt",
    soilLabel: "Silty Soil",
    color: "#f39c12",
    lightColor: "#fefaf2",
    borderColor: "#f39c12",
    crop: "Wheat, corn, general grain crops",
    riskLevel: "Medium",
    data: {
      soil_moisture: 0.50,
  ...
  },
  {
    ...
    lightColor: "#f2fdf5",
    borderColor: "#27ae60",
    crop: "Rice, brassicas, some root crops",
    riskLevel: "Low",
    data: {
      soil_moisture: 0.70,
...
// ── Weather parameters from weather_adapter.py ───────────────────────────────
const WEATHER_PARAMS = [
  {
    ...
    default: 51.44,
    unit: "°",
    description: "Location latitude for weather data fetch",
    step: 0.01,
  },
  {
    key: "longitude",
    ...
    default: 5.47,
    unit: "°",
    description: "Location longitude for weather data fetch",
    step: 0.01,
  },
  {
    key: "force_rainy",
    ...
    type: "toggle",
    default: false,
    description: "Override weather API and force rainy conditions",
  },
  {
    key: "update_period_s",
  ...
  {
    key: "rain_mm_threshold",
    label: "Rain Threshold",
    type: "slider",
    default: 0,
    unit: "mm/h",
    description: "Minimum rain (mm/h) to consider as rainy",
    min: 0,
    max: 20,
    step: 0.5,
  },
  {
    key: "visibility_m_threshold",
    label: "Visibility Threshold",
    type: "slider",
    default: 500,
    unit: "m",
    description: "Minimum visibility (m) before triggering caution",
    min: 100,
    max: 5000,
    step: 100,
  },
  {
    key: "wind_ms_threshold",
    label: "Wind Threshold",
    type: "slider",
    default: 12,
    unit: "m/s",
    description: "Wind speed above which conditions are flagged",
    min: 0,
    max: 30,
    step: 0.5,
  },
  {
    key: "speed_scale_rain",
    label: "Speed Scale (Rain)",
    type: "slider",
    default: 0.6,
    unit: "×",
    description: "Robot speed multiplier when raining",
    min: 0.1,
    max: 1.0,
    step: 0.05,
  },
  {
    key: "stop_add_rain",
    label: "Stop Distance (Rain)",
    type: "slider",
    default: 0.15,
    unit: "m",
    description: "Extra obstacle stop distance added when raining",
    min: 0,
    max: 1.0,
    step: 0.05,
  },
  {
    ...
    default: 3,
    unit: "s",
    description: "Timeout for Open-Meteo API requests",
    min: 1,
    max: 30,
  },
  {
    key: "max_stale_s",
    label: "Max Stale Duration",
    type: "number",
    default: 300,
    unit: "s",
    description: "How long to use cached weather before fallback",
    min: 60,
    max: 3600,
  },
];
...
function HomePage({ onNavigate }) {
  ...
  const cards = [
    {
      ...
      desc: "Adjust rain, wind, visibility thresholds and robot safety parameters. Force rain mode for live demos.",
      accent: "#3b82f6",
      light: "#eff6ff",
    },
    {
      ...
      desc: "Inspect sandy, silty and clay zones. View soil moisture, temperature, nitrogen levels and crop health.",
      accent: "#16a34a",
      light: "#f0fdf4",
    },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center" }}>
            ...
            <span style={{
              ...
              fontSize: 22, fontWeight: 700, color: "#111827", letterSpacing: "-0.02em"
            }}>FarmSight</span>
            <span style={{
      ...
      {/* Hero */}
      <div style={{ padding: "64px 48px 32px", textAlign: "center", maxWidth: 640, margin: "0 auto" }}>
        <p style={{
          fontSize: 13, fontWeight: 600, color: "#16a34a",
          letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16
        }}>Digital Twin · Precision Agriculture</p>
        <h1 style={{
          fontFamily: "'Playfair Display', serif",
          fontSize: 42, fontWeight: 700, color: "#111827",
          lineHeight: 1.15, letterSpacing: "-0.03em", margin: "0 0 16px"
        }}>Field Intelligence<br />Dashboard</h1>
      ...
      <div style={{
        ...
        gap: 24, maxWidth: 860, margin: "32px auto 0", padding: "0 48px 64px"
      }}>
        {cards.map(card => (
          <div
            key={card.id}
          ...
          >
            ...
            <p style={{ fontSize: 12, color: card.accent, fontWeight: 600, margin: "0 0 14px", letterSpacing: "0.02em" }}>
              {card.subtitle}
            </p>
            <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.65, margin: 0 }}>{card.desc}</p>
            <div style={{
              marginTop: 24, display: "flex", alignItems: "center", gap: 6,
              color: card.accent, fontSize: 13, fontWeight: 600
            }}>
      ...
      <div style={{
        ...
        padding: "10px 48px", display: "flex", gap: 24, alignItems: "center"
      }}>
        <span style={{ fontSize: 11, color: "#9ca3af", letterSpacing: "0.05em", textTransform: "uppercase", fontWeight: 600 }}>
          System Status
        </span>
        {["ROS 2 Connected", "Open-Meteo Active", "3 Zones Monitored"].map(s => (
          <span key={s} style={{ fontSize: 11, color: "#16a34a", display: "flex", alignItems: "center", gap: 5 }}>
...
function PageHeader({ icon, title, subtitle }) {
  return (
    <div style={{
      ...
      <div style={{ width: 200 }} /> {/* spacer for back button */}
      <div style={{ flex: 1, textAlign: "center" }}>
        ...
        <p style={{ fontSize: 12, color: "#9ca3af", margin: "2px 0 0", letterSpacing: "0.03em" }}>{subtitle}</p>
      </div>
      <div style={{ width: 200 }} />
    </div>
  );
          ...
          {ZONES.map(zone => (
            <div
              ...
              style={{
                ...
                boxShadow: hovered === zone.id
                  ? `0 16px 48px ${zone.color}28`
                  : "0 2px 8px rgba(0,0,0,0.06)",
              }}
            >
              {/* Soil texture header */}
              ...
              <div style={{ padding: "20px 24px 24px" }}>
                <p style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.6, margin: "0 0 20px" }}>
                  {zone.description}
                </p>
                ...
                <div style={{
                  ...
                  color: zone.color, fontSize: 12, fontWeight: 700
                }}>
                  View Details →
                </div>
              </div>
...
function ZoneDetailPage({ zone, onBack }) {
  ...
  function simulate() {
    ...
    const wind = weatherEffect.wind;
    const temp = weatherEffect.temp;

    const sensMap = { sand: 0.02, silt: 0.05, clay: 0.08 };
    const gain = sensMap[zone.soil];

    let moisture = base.soil_moisture + rain * gain - wind * 0.003;
    moisture = Math.max(0, Math.min(1, moisture));

    let cropMoisture = base.crop_moisture + rain * gain * 0.7;
    if (zone.soil === "clay" && moisture > 0.88) cropMoisture *= 0.6;
    cropMoisture = Math.min(1, cropMoisture);

    const tempDelta = (temp - 20) * (zone.soil === "sand" ? 0.6 : zone.soil === "silt" ? 0.3 : 0.1);
    const soilTemp = base.soil_temperature + tempDelta - rain * 0.1;

    let nitrogen = base.nitrogen_level - rain * (zone.soil === "sand" ? 0.04 : zone.soil === "silt" ? 0.01 : 0.005);
    nitrogen = Math.max(0, Math.min(1, nitrogen));
    ...
    const risk = factors.length >= 2 ? "High" : factors.length === 1 ? "Medium" : "Low";

    setSimulated({ moisture, cropMoisture, soilTemp, nitrogen, drainage: base.drainage_rate, risk, factors });
  }

  const display = simulated || zone.data;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <PageHeader
        icon="🔬"
        title={zone.name}
        subtitle={`${zone.soilLabel} · ${zone.crop}`}
      />
      ...
      <div style={{ padding: "40px 48px", maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          ...
          <div style={{
            ...
            <div style={{
              height: 100, background: SOIL_TEXTURES[zone.soil], position: "relative"
            }}>
              <div style={{
                position: "absolute", inset: 0,
              ...
              <div style={{
                position: "absolute", bottom: 16, left: 20, right: 16,
            ...
            <div style={{ padding: "24px" }}>
              <DataRow
                label="Soil Moisture"
                value={simulated ? simulated.moisture : display.soil_moisture}
                bar barColor={zone.color}
              />
              <DataRow
                label="Soil Temperature"
                value={simulated ? simulated.soilTemp : display.soil_temperature}
                unit="°C"
              />
              <DataRow
                label="Nitrogen Level"
                value={simulated ? simulated.nitrogen : display.nitrogen_level}
                bar barColor="#8b5cf6"
              />
              <DataRow
                label="Crop Moisture"
                value={simulated ? simulated.cropMoisture : display.crop_moisture}
                bar barColor="#06b6d4"
              />
              <DataRow
                label="Drainage Rate"
                value={simulated ? simulated.drainage : display.drainage_rate}
                bar barColor="#f59e0b"
              />
              ...
              {simulated && simulated.factors.length > 0 && (
                <div style={{
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#dc2626", marginBottom: 8, letterSpacing: "0.05em" }}>
                    ⚠ RISK FACTORS
                  </div>
                  {simulated.factors.map(f => (
                    <div key={f} style={{ fontSize: 12, color: "#7f1d1d", marginBottom: 4 }}>• {f}</div>
                  ))}
                </div>
              )}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{
              ...
              <div style={{
                padding: "14px 16px", background: zone.lightColor,
                borderRadius: 12, border: `1px solid ${zone.color}22`, marginBottom: 12
              }}>
                <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 4 }}>
                  Recommended Crops
                </div>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{zone.crop}</div>
              </div>
            ...
            <div style={{
              ...
              <h3 style={{
                ...
                fontSize: 16, fontWeight: 700, color: "#111827",
                margin: "0 0 4px"
              }}>Weather Impact Simulation</h3>
              <p style={{ fontSize: 12, color: "#9ca3af", margin: "0 0 20px" }}>
                Adjust parameters to see how weather affects this zone
              </p>
                ...
                <div key={param.key} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#111827" }}>
                      {weatherEffect[param.key]} {param.unit}
                    </span>
                  </div>
                  <input
                    type="range"
...
function WeatherPage({ onBack }) {
  const [values, setValues] = useState(
    Object.fromEntries(WEATHER_PARAMS.map(p => [p.key, p.default]))
  );
      ...
      <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 28 }}>{isRainy ? "🌧" : "☀️"}</span>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>
                {isRainy ? "Rainy Conditions Active" : "Clear Conditions"}
              </div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>
                {isRainy ? "Robot speed reduced to 60%, stop distance increased" : "Normal robot operation parameters"}
              </div>
            </div>
          </div>
          <div style={{
          ...
          {WEATHER_PARAMS.map(param => (
            <div key={param.key} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>{param.label}</div>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>{param.description}</div>
                </div>
                {param.type !== "toggle" && (
                  <span style={{
                    fontSize: 13, fontWeight: 700, color: "#3b82f6",
                    background: "#eff6ff", padding: "2px 10px", borderRadius: 8
                  }}>
              ...
              {param.type === "toggle" && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
                  <span style={{ fontSize: 13, color: values[param.key] ? "#dc2626" : "#6b7280", fontWeight: 600 }}>
                    {values[param.key] ? "🌧 Rain forced ON — overrides live API" : "Uses live Open-Meteo weather data"}
                  </span>
                  <div
